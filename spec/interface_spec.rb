...
RSpec.describe Interface do
  ...
  describe "#ask_for_guess" do
    it "prints a message asking for a guess" do
      ...
      allow(game).to receive(:valid_guess?).and_return(true)
      expect(view).to receive(:print).with(/What's your next guess?/)
      view.ask_for_guess
    end

    it "returns a valid guess" do
      expect(view).to receive(:gets).and_return("A\n")
      expect(game).to receive(:valid_guess?).with("A").and_return(true)
      expect(view.ask_for_guess).to eq "A"
    end

    it "converts guesses to uppercase" do
      expect(view).to receive(:gets).and_return("iNpUt\n")
      expect(game).to receive(:valid_guess?).with("INPUT").and_return(true)
    ...
    it "prints an error if a guess is invalid" do
      ...
      expect(game).to receive(:valid_guess?).and_return(false, true)
